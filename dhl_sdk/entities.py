    ...
    def _unpack_reference_entities(cls, data) -> dict:
        ...
        def unpack_entity(source: str, entity):
            ...
            for i, info in enumerate(data[source]):
                ...
                var = entity.requests(data["client"]).get(id)
                unpacked.append(var)
...
class CultivationPropagationModel(CultivationModel):
    ...
    def predict(
        ...
        model_confidence: int, optional
            ...
            expected to fall, with a specified level of certainty, i.e, setting it to 80% corresponds
            to capturing the range between the 10th and 90th percentiles of the model's output.
            Must be a value between 1 and 99, by default 80
